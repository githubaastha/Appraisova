...
interface Props {
    ...
    onClose: () => void
    onSave: (data: { deptName: string; deptDescription: string }) => void
}

export default function EditDepartmentModal({ department, onClose, onSave }: Props) {
    const [deptName, setDeptName] = useState(department.deptName)
    const [deptDescription, setDeptDescription] = useState(department.deptDescription ?? '')
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl border border-gray-200 p-6 w-full max-w-md flex flex-col gap-4 shadow-lg">
                ...
                <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold text-gray-800">Edit Department</p>
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    <input
                        ...
                        className="border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:border-[#1089D3] focus:ring-1 focus:ring-[#1089D3] transition-all"
                    />
